...
class TitleScreen(screen.Screen):
    def __init__(self):
        ...
        self.entities =[message.Message((int(0.45 * self.width), int(0.4375 * self.height)), "Play", True, int(0.03 * self.width)),
          message.Message((int(0.45 * self.height), int(0.5625 * self.height)), "Quit", False, int(0.03 * self.width))]

    def render(self, surf):
        surf.blit(self.big_font.render("Tron", False, (66, 134, 244)), (int(0.35 * self.width), int(0.25 * self.height)))
        ...
        if event.key == 273:
            for a in range(len(self.entities)):
                if self.entities[a].highlighted:
                    ...
                    self.entities[(a - 1) % 2].highlighted = True
                    break
        if event.key == 274:
